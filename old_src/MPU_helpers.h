// I DO NOT think Wire.endTransmission();
//   in calcYawPitchRoll() is in the correct place
...
void testAndCalibrateIMU () {
  uint8_t c = readByte(MPU6050_ADDRESS, WHO_AM_I_MPU6050);  // Read WHO_AM_I register for MPU-6050
  if (c == 0x68) {  
      Serial.println("MPU6050 is online...");
    
      MPU6050SelfTest(SelfTest); // there's lots of these we can test, see original code
...
void calcYawPitchRoll() {
  ...
  // owen addition
  // I DO NOT think this is in the right place
  Wire.endTransmission(); 
  delayMicroseconds(2); 

  // all the code below was original outside of if(readByte(MPU6050_ADDRESS block
  Now = micros();
